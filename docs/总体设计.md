...
class BlockList {
    ...
    std::vector<BlockIndexNode<Key>> index_table; 
    
public:
    BlockList(string filename);
    
    void insert(const Key &key, const Value &val);
    void find(const Key &key, vector<Value> &res);
    void remove(const Key &key);
    
    void split(int node_index, Node &node); 
    void merge(int node_index, Node &node); 
};
